...
- **Purpose**: Contains the implementation of the four search algorithms: BFS, DFS, A*, and Best-First Search.
- **Key Components**:
    - **Breadth-First Search (BFS)**: Implements the BFS algorithm using a queue (FIFO).
    - **Depth-First Search (DFS)**: Implements the DFS algorithm using a stack (LIFO).
    - **A***: Implements the A* search, utilizing both cost and heuristic values to guide the search.
    - **Best-First Search**: Implements the Best-First search algorithm, prioritizing nodes based on the heuristic value.
...
- **Purpose**: Automates the process of testing the search algorithms across multiple problem instances. It reads problem descriptions from a set of JSON files and runs each algorithm to evaluate its performance in terms of path found, cost, nodes generated, and execution time.
- **Key Components**:
    - `run_tests()`: Executes all the search algorithms on predefined road network files.
    - `compare_results()`: Compares the performance of the algorithms.
